...
핵심 요약
- map은 "키 -> 값" 구조다.
- countMap[item]++ 는 없던 키면 value의 기본값에서 시작한다.
- 이 예제는 value 타입이 int이므로 기본값이 0처럼 동작한다.
- 조회만 할 때는 find()로 확인하는 습관이 좋다.
------------------------------------------------------------
주의 포인트
- []는 삽입/수정에는 매우 편리하다.
- 하지만 단순 조회 상황에서는 없는 키를 만들 수 있으므로 find()가 더 안전하다.
- 즉, map의 value가 "항상 0"이 되는 것은 아니고,
  value 타입의 기본값으로 시작한다고 이해하는 것이 정확하다.
------------------------------------------------------------
*/
...
int main() {
    ...
    for (vector<string>::const_iterator it = soldItems.begin(); it != soldItems.end(); ++it) {
        ...
        // 만약 *it에 해당하는 key가 아직 없으면 새 항목이 만들어진다.
        // 이 예제에서 value 타입은 int이므로 기본값 0에서 시작하고,
        // 그 다음 ++ 되어 1, 2, 3 ...처럼 집계된다.
        salesCount[*it]++;
    }
