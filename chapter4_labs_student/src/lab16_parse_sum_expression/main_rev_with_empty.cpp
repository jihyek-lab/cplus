...
/*
[Lab16 - With Empty Tokens]
------------------------------------------------------------
목표
- "7+23+5+100" 같은 문자열을 입력 받아 합을 계산한다.

예외 처리 대상
- 빈 토큰이 생길 수 있는 입력을 방어한다.
  예) "2+3+", "+2+3", "2++3"
- 빈 토큰은 무시한다.
...
처리 흐름
1) find로 '+' 위치를 찾는다.
2) substr로 토큰을 잘라내고, 비어있지 않을 때만 stoi로 더한다.
3) 더 이상 '+'가 없으면 마지막 토큰을 동일하게 처리하고 종료한다.
------------------------------------------------------------
*/
...
int main() {
    ...
    while (true) {
        ...
        if (plus == -1) { // 더 이상 '+' 없음 -> 마지막 토큰
            ...
            if (!tok.empty()) sum += stoi(tok); // 빈 토큰 방어
            break;
        }
